...
def print_game_state(game: LiarsDiceGame, current_player: int, reveal_all: bool = False):
    ...
    if game.current_bid is None:
        ...
        print(f"{quantity} {value}'s (made by Player {game.previous_player})")
...
def get_valid_action_choice(valid_actions: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # Then list bid options in a horizontal grid layout
    if bid_actions:
        print(f"\n{Colors.BOLD}Bid Options:{Colors.RESET}")
        # Group bids by value for easier selection
        values = sorted(set(a['value'] for a in bid_actions))
        
        # Display bid value headers horizontally
        header = "    "
        for value in values:
            header += f"{Colors.CYAN}Value {value}{Colors.RESET}    "
        print(header)
        
        # Get max quantity for each value
        max_quantities = {}
        for value in values:
            max_quantities[value] = max(a['quantity'] for a in bid_actions if a['value'] == value)
        
        # Display quantities in a grid
        row_max = max(max_quantities.values())
        for quantity in range(1, row_max + 1):
            row = f"{quantity:2d}: "
            for value in values:
                # Check if this quantity-value combination is valid
                found = False
                for action in bid_actions:
                    if action['quantity'] == quantity and action['value'] == value:
                        action_index += 1
