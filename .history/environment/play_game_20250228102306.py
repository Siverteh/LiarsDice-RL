...
def get_valid_action_choice(valid_actions: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # Then list bid options horizontally
    if bid_actions:
        ...
        for value in values:
            print(f"\n{Colors.CYAN}Value: {value}{Colors.RESET}")
            quantities = sorted(a['quantity'] for a in bid_actions if a['value'] == value)
            
            # Display horizontally with 4 options per row
            row = ""
            count = 0
            for quantity in quantities:
                action_index += 1
                key = str(action_index)
                bid_text = f"{key}. Bid {quantity} {value}'s"
                row += f"{bid_text:<20} "  # Pad to 20 chars for alignment
                count += 1
                ...
                # Start a new row after 4 options
                if count % 4 == 0:
                    print(row)
                    row = ""
            
            # Print any remaining options in the last row
            if row:
                print(row)
