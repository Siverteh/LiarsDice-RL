...
def play_interactive_game():
    ...
    while not done:
        ...
        # Special handling for challenge actions
        if action['type'] == 'challenge':
            # Store the bid details before they get reset
            if game.current_bid is not None:
                bid_quantity, bid_value = game.current_bid
                
                # Execute the action
                next_observations, rewards, done, info = game.step(action)
                
                # Show the game state after the challenge
                print_game_state(game, current_player, reveal_all=True)
                
                # Count the actual dice matching the bid
                total_matching = 0
                ...
                if not bid_successful:  # Challenge was correct
                    result = f"{Colors.GREEN}Challenge successful!{Colors.RESET}"
                    loser = game.previous_player if game.previous_player is not None else -1
                else:  # Challenge was incorrect
                ...
                print(f"{Colors.RED}Error: No current bid to challenge.{Colors.RESET}")
                input("Press Enter to continue...")
        else:
            # For non-challenge actions, process normally
            next_observations, rewards, done, info = game.step(action)
            observations = next_observations
