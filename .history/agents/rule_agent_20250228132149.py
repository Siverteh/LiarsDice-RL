...
class MixedAgent(BaseAgent):
    def __init__(self, player_id, random_ratio=0.8):
        super().__init__(player_id)
        self.random_agent = RandomAgent(player_id)
        self.conservative_agent = VeryMildConservativeAgent(player_id)
        self.random_ratio = random_ratio
        
    def act(self, observation, valid_actions):
        if random.random() < self.random_ratio:
            return self.random_agent.act(observation, valid_actions)
        else:
            return self.conservative_agent.act(observation, valid_actions)
